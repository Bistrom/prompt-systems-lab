#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Etap 1 pipeline'u dziennego przez Ollama API — v7.3.3: krótki kontrakt + cache + trwały audyt surowych odpowiedzi."""
...
STAGE1_CONTRACT_VERSION = "stage1_v7_3_3_short_extraction_contract_raw_cache_audit_2026-04-25"
...
# Tylko ostrzeżenia, NIE powód do zastąpienia partial reportu.
# v8 było zbyt agresywne; v7.3.1 nie ma psuć przepływu.
...
def build_prompt(chunk_text: str, chunk_index: int, chunk_count: int, day: str) -> str:
    return f"""Jesteś etapem 1 lokalnego pipeline'u dziennego.

TWOJE ZADANIE:
Wydobądź fakty robocze z jednego fragmentu rozmowy. Nie twórz finalnego daily loga.
...
- Nie przepisuj instrukcji technicznych jako faktów.
- Nie zgaduj brakujących działań, plików ani lokalizacji.
- Nie wymyślaj nazw folderów.
...
- Pisz po polsku, krótko i operacyjnie.
...
def compute_cache_key(chunk_text: str, model: str) -> str:
    h = hashlib.sha256()
...
def process_chunk(chunk_index: int, chunk_count: int, chunk_dir: Path, partial_dir: Path, day: str, model: str, ollama_url: str, cache_dir: Path | None, audit_dir: Path | None) -> dict[str, object]:
    ...
    cache_key = compute_cache_key(chunk_text, model)
    ...
    prompt = build_prompt(chunk_text, chunk_index, chunk_count, day)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Etap 1 pipeline'u dziennego — krótka ekstrakcja + cache.")
    ...
    print("[etap1] Wersja: v7.3.3 short-extraction-cache-raw-audit")
    ...
    for i in range(1, args.chunk_count + 1):
        try:
            record = process_chunk(i, args.chunk_count, chunk_dir, partial_dir, args.day, args.model, args.ollama_url, cache_dir, audit_dir)
